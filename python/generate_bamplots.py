...
def sam_bam_cram(seqfile):
    ...
    """
    with open(seqfile, 'rb') as iff:
         x = iff.read(4)
    if x == b'CRAM':
        return 'c'
    elif x[:3] == b'\x1f\x8b\x08':
        # Could be a regular gzip file. Ensure it's actually a bam
        with gzip.open(seqfile, 'rb') as iff:
            x = iff.read(4)
        if x == b'BAM\x01':
            return 'b'
        else:
...
def get_mapping_data(seqfile, num_threads, reference_file=None):
    ...
    file_type = sam_bam_cram(seqfile)
    samfile = pysam.Samfile(seqfile, 'r' + file_type, reference_filename=reference_file)
    try:
        samfile.check_index()
    except ValueError as e:
        if str(e) == 'mapping information not recorded in index or index not available':
            indexed = False
        else:
            raise
    else:
    ...
    if indexed and num_threads > 1:
        ...
        if num_threads > 1:
            print('Although `--threads` was provided and >1 ({}), the {} file is not indexed so '
                  'we can only continue in single-threaded mode.'.format(
                        num_threads, 
                        ('bam' if file_type == 'b' else 'cram' if file_type == 'c' else 'sam')))
...
def main():
    ...
    parser.add_argument('--skip_insert_sizes', help='Skip plotting the insert size  profile.',
                        action='store_true')
    params = parser.parse_args(['IPICRC023.T1.rna.teff.trimmed.rrna.sorted.cram',
                                '--reference_file', '/krummellab/data1/ipi/data/refs/hg38_files/hg38.fa',
                                '--threads', '1'])
