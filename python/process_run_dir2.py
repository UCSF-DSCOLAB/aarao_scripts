...
def main():
    ...
    parser.add_argument('--genome_build', help='Genome build used', type=str,
                        required=False, default='GRCh38')
    ...
    if os.path.exists(os.path.join(run_dir, 'cell_counts.tsv')):
        cell_counts = pd.read_csv(os.path.join(run_dir, 'cell_counts.tsv'), sep='\t', header=0, 
                                  index_col=0)
        assert ({'well', 'submission_name', 'submission_date', 'cell_count', 'stain_version'} <= 
                    set(cell_counts.columns)), "Invalid cell_counts.tsv file provided."
        cell_counts = cell_counts[['well', 'submission_name', 'submission_date', 'cell_count', 'stain_version']]
        failed_samples = [x for x in cell_counts.index if x not in info.index]
        if failed_samples:
            for f in failed_samples:
                logging.warning('Sample {} was seen in cell_counts.tsv but not in the output '
                                'directory and is assumed to have failed sequencing.'.format(f))
                cell_counts.drop(f, axis=0, inplace=True)
    ...
    info['genome_build'] = params.genome_build
    info['pipeline_version'] = params.pipeline_version
    info['pipeline_description'] = 'automated'
    info['quant_method'] = 'alignment'
    ...
    for sample in rnaseq_table.index:
        rnaseq_table.loc[sample].to_csv(os.path.join(run_dir, 'output', sample, 
                                                     '{}_rnaseq_table.tsv'.format(sample)), 
