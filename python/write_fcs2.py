...
def write_fcs(filename, chn_names, chn_names_short, data,
              text_kw_pr={},
              endianness=None,
    ...
    chn_names_short: list of str, length C
        Short names of the output channels
    ...
    assert len(chn_names_short) == data.shape[1], msg
    ...
    DATA = struct.pack('>{0}{1}'.format(len(data1), DATAYPE.lower()), *data1)
    ...
    TEXT += '/$BYTEORD/{0}/$DATATYPE/{1}'.format(byteord, DATAYPE)
    ...
    for jj in range(data.shape[1]):
        ...
        data_str = '/'.join(['',
                            '$P{0}B'.format(jj+1),
                            str(_text_kw_pr.pop('$P{0}B'.format(jj+1), bits_used)),
                            '$P{0}E'.format(jj+1),
                            str(_text_kw_pr.pop('$P{0}E'.format(jj+1), '0,0')),
                            '$P{0}N'.format(jj+1),
                            str(_text_kw_pr.pop('$P{0}N'.format(jj+1), chn_names_short[jj])),
                            '$P{0}S'.format(jj+1),
                            str(_text_kw_pr.pop('$P{0}S'.format(jj+1), chn_names[jj])),
                            '$P{0}R'.format(jj+1),
