...
then
  ...
  echo "USAGE: spawn_download_from_STRATUS.sh <run_mode> <download_manifest>"
  exit
else
...
then
  ...
  exit 1
elif ! grep -q "stratus.ucsf.edu" ~/.netrc
then
  echo "ERROR: ~/netrc does not contain a record for stratus.ucsf.edu. Cannot continue without proper credentials for that server."
  echo "A proper netrc record looks like:"
  echo """
machine stratus.ucsf.edu
        login genomicsUser
        password <PASSWORD>
"""
  exit 1
...
then
    ...
    bash download_from_STRATUS.sh 1>${log_out} 2>${log_err}
elif [[ "${1}" == "slurm" ]]
then
    echo NotImplemented
elif [[ "${1}" == "qsub" ]]
