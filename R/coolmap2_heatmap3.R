...
coolmap.2 <- function (x, cluster.by = "de pattern", col = NULL, linkage.row = "complete", 
                       linkage.col = "complete", show.dendrogram = "both", ...) 
{
    x <- as.matrix(x)
    nsamples <- ncol(x)
    if (nsamples < 2L) 
        stop("Need at least 2 rows and 2 columns")
    ...
    show.dendrogram <- match.arg(show.dendrogram, c("both", "row", 
        "column", "none"))
    suppressPackageStartupMessages(OK <- requireNamespace("gplots", 
        quietly = TRUE))
    if (!OK) 
        stop("gplots package required but not installed (or can't be loaded)")
    linkage.row <- as.character(linkage.row)
    if (linkage.row %in% c("w", "wa", "war", "ward")) 
        linkage.row <- "ward.D2"
    METHODS <- c("none", "ward.D", "single", "complete", "average", 
        "mcquitty", "median", "centroid", "ward.D2")
    linkage.row <- match.arg(linkage.row, METHODS)
    linkage.col <- as.character(linkage.col)
    if (linkage.col %in% c("w", "wa", "war", "ward")) 
        linkage.col <- "ward.D2"
    linkage.col <- match.arg(linkage.col, METHODS)
    if (linkage.col == "none") {
        hc <- FALSE
        if (show.dendrogram == "both") 
            show.dendrogram <- "row"
        if (show.dendrogram == "column") 
            show.dendrogram <- "none"
    }
    else {
        hc <- as.dendrogram(hclust(dist(t(x), method = "euclidean"), 
            method = linkage.col))
    }
    if (cluster.by == "de pattern") {
    ...
    if (linkage.row == "none") {
        ...
        if (show.dendrogram == "row") 
            show.dendrogram <- "none"
    }
    else {
        hr <- as.dendrogram(hclust(dist(x, method = "euclidean"), 
            method = linkage.row))
    }
    if (length(col) == 1L) 
        col <- switch(col, redblue = gplots::colorpanel(256, 
            "blue2", "white", "red2"), redgreen = gplots::colorpanel(256, 
